...
class ResizableImage:
    def __init__(self, path: str):
        width, height, channel, data = gui.load_image(path)
        with gui.texture_registry(show=False):
            img = gui.add_static_texture(width, height, default_value=data)
        self.image = gui.add_image(img)
        self.orig_width = width
        self.orig_height = height

    def update_proportional_scale(self,  max_width: int, max_height: int):
        by_width_ratio = max_width / self.orig_width
        by_width_height = by_width_ratio * self.orig_height

        if by_width_height > max_height:
            by_height_ratio = max_height / self.orig_height
            by_height_width = by_height_ratio * self.orig_width
            
            new_width = by_height_width
            new_height = max_height
        else:
            new_width = max_width
            new_height = by_width_height
            
        gui.configure_item(self.image, width=new_width, height=new_height)
...
class Grapher(AppComponent):
    def __init__(self, identifier: str, scaling_factor: float) -> None:
        ...
        self.image_rocket: ResizableImage = None
        self.image_grid: ResizableImage = None
    ...
    def update(self, iliad: IliadDataController) -> None:
        ...
        region = gui.get_available_content_region(self.tracking_tab)
        plot_width = int(region[0] / 3)
        plot_height = int(region[1] / 2)
        ...
        self.image_rocket.update_proportional_scale(plot_width, plot_height)
        self.image_grid.update_proportional_scale(plot_width, plot_height)
